...
namespace ClipsFormsExample
{
    public partial class ClipsFormsExample : Form
    {
        ...
        private List<string> addedFacts;
        ...
        {
            outputBox.Text += "\r\n" + (sender as Button).Text + $" - {CF_TB.Text}" + "\r\n";
            addedFacts.Add('|' + (sender as Button).Text + $"|{CF_TB.Text}" + "\r\n");
        }
        ...
        private void AddFact_B_Click(object sender, EventArgs e)
        {
            foreach (var line in addedFacts)
            {
                if (line.Count() < 1) continue;
                if (line[0] != '|') continue;
                var temp = line.Split('|').ToList();
                clips.Eval($"(assert (add-fact (fact \"{temp[1]}\") (confidence {temp[2]})))");
                clips.Run();
                clips.Run();
                HandleResponse();
            }
